...
class piecewiseXrange(list):
    ...
    def my_iter(self):
        ...
        if self._xrangeMode:
            return self.xrange_iter
        else:
            return self.__iter__ 
    
    def xrange_iter(self):  #use a generator to iterate
...
def maybe_xrange_Iter(lst):
    """If lst is a piecewiseXRange, return lst.my_iter()
    else retyrn lst.__iter__()
    """
...
def iterate_xrange_limit(pwx, limit):
    """    
    Chunk piecewiseXrange *pwx* into individual pieceWiseXranges defining no more than limit elements 
    Iterator returns individual chunks
    """
